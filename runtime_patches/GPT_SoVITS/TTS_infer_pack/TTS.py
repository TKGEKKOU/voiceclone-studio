...
class TTS:
    ...
    def set_ref_audio(self, ref_audio_path: str):
        ...
        """
        # Reuse cached features when the same reference is set again
        # (e.g. switching back to a previously used character).
        if (
            self.prompt_cache.get("ref_audio_path") == ref_audio_path
            and self.prompt_cache.get("prompt_semantic") is not None
            and self.prompt_cache.get("refer_spec") not in [[], None]
        ):
            return
